...
func update():
    label.text = str(DeckManager.draw_pile.size())

    if DeckManager.draw_pile.size() <= 0:
        stack.visible = false
    ...
    var stack_count = clamp(floor(DeckManager.draw_pile.size() / COUNT_BETWEEN_STACKS), 0, 10)

    v_box_container.set("theme_override_constants/separation", INIT_SEPARATION + (INCREMENT * stack_count))

func _on_button_pressed() -> void :
    SignalBus.ask_to_show_draw_pile.emit()
